...
def copy_yml_file(src_path, dest_dir, step):
    # 确保目标目录存在，如果不存在则创建
    ...
    # 拼接 step 到文件名
    new_file_name = f"{file_name}_step_{step}{file_ext}"
    ...
    print(f"YAML file copied to {dest_path}")
...
def tensor2img(tensor, out_type=np.uint8, min_max=(0, 1)):
    ...
    # print("n_dim, ", n_dim)
    if n_dim == 4:
        n_img = len(tensor) # batch, channel, h, w
