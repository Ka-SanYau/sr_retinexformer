...
def load_image_pillow(image_path):
    """Load an image as a NumPy array with pixel values in [0, 255] and 'HWC' format."""
    img = Image.open(image_path).convert('RGB')  # Convert to RGB to ensure 3 channels
    img_array = np.array(img, dtype=np.uint8)  # Convert image to NumPy array with dtype uint8
    return img_array  # This will be in HWC format
